...
function Profile_navbar() {
  ...
  const openCity = (evt, cityName) => {
    ...
    for (i = 0; i < tablinks.length; i++) {
      tablinks[i].className = tablinks[i].className.replace("border-[#BF5A36] rounded-tr-lg rounded-tl-lg border-r-2 border-b-0 border-l-2 border-t-2", "");
    }
    document.getElementById(cityName).style.display = "block";
    evt.currentTarget.className += " border-[#BF5A36] rounded-tr-lg rounded-tl-lg border-r-2 border-b-0 border-l-2 border-t-2";
  };
